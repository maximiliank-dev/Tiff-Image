...
class TiffWriterHeader {
   ...
   public:
    ...
    TiffWriterHeader(std::basic_ostream<char>& stream,
                     const tiff_header_endian endian)
        : _endian(endian), _stream(stream) {}
    TiffWriterHeader(std::basic_ostream<char>& stream,
                     const tiff_header_endian endian, const uint_t idf_offset)
        : _endian(endian), _idf_offset(idf_offset), _stream(stream) {}
...
class TiffWriteData {
    // type checking
    static_assert(std::is_integral_v<TP> && std::is_unsigned_v<TP>,
                  "TP must be of unsigned type");
   ...
   protected:
     ...
     */
    struct less_tiff {
        constexpr bool operator()(const TiffTagType& lhs,
                                  const TiffTagType& rhs) const {
   ...
   public:
     ...
     *         rational_values: data structure to store the values of ags with
     * data type RATIONAL
     */
    void write_ifd_element(TiffTagType tag, TiffDataVariant variant,
        ...
        std::array<char, 2> tag_array =
            this->_endian_handler->convert_to_array(tag_short);
        write_char<2>(tag_array, this->_stream);

        // data type
        std::array<char, 2> type_array =
            this->_endian_handler->convert_to_array(
                static_cast<ushort_t>(type));
        ...
        // value (in offset)
        std::visit(
            [this, &rational_values](auto& el) -> void {
                ...
                if constexpr (std::is_same<T, std::string>::value) {
                    ...
                    std::is_same<T, std::vector<uint_t>>::
                        value) {  // use the implemented array conversion
                                  // function
                    ...
                    if (el.size() == 0) {
                        ...
                        // move the next entry towards typesize * number of
                        // elements written
                        this->_offset += type_size * el.size();
                    }
                } else if constexpr (std::is_same<T, std::vector<uint64_t>>::
                                         value) {
                    // data count
                    std::array<char, 4> tag_count =
                        this->_endian_handler->convert_to_array(
                    ...
                    // move the next entry towards typesize * number of elements
                    // written
    ...
    void write_IFDs() {
        ...
        // add the number of bytes of the IFD size STRIP offset and
        // StripByteCount also add the "Next IFD offset"(4 Bytes) byte count and
        // the Number of "Directory Entries"(2 bytes) unit is in 'number of
        // bytes' Formula: start position + (number of ifd)*Tag Size +
        // (Directory Entries) + NumIFD(2) + NextIFD(4)
        this->_offset = static_cast<uint64_t>(offset_tiff_data) +
                        (ifd_cnt)*TiffTag_size + 2 +
                        4;  // 182 = 32 + 12*12 + 2 + 4
        ...
        this->_values[TiffTagType::StripOffsets] =
            make_variant(TiffTagType::StripOffsets, this->_offset_data);
        ...
        std::array<char, 4> tmp4 =
            this->_endian_handler->convert_to_array(uint_t{0x0});
            ...
            std::visit(
                [this](auto& vec) {
                    ...
                    for (auto& element : vec) {
                        ...
                        } else if constexpr (std::is_same_v<element_type,
                                                            uint8_t>) {
        ...
        std::array<char, 4> offset_idf_header =
            this->_endian_handler->convert_to_array(
...
void TiffWriteData<TP>::tags_to_set() {
    this->_values.clear();
    this->_values.emplace(
        TiffTagType::ImageWidth,
        make_variant(TiffTagType::ImageWidth, {this->_img->get_width()}));
    this->_values.emplace(
        TiffTagType::ImageLength,
        make_variant(TiffTagType::ImageLength, {this->_img->get_height()}));
    this->_values.emplace(TiffTagType::Compression,
                          make_variant(TiffTagType::Compression, {1}));

    this->_values.emplace(
        TiffTagType::PhotometricInterpretation,
        make_variant(TiffTagType::PhotometricInterpretation, {0x0001}));
    this->_values.emplace(
        TiffTagType::RowsPerStrip,
        make_variant(TiffTagType::RowsPerStrip, {this->_img->get_height()}));
    this->_values.emplace(TiffTagType::StripByteCounts,
                          make_variant(TiffTagType::StripByteCounts,
                                       {this->_img->get_total_size()}));

    // value will be overwritten in writeIDFs method
    // only here, that the amount of IFDs match
    this->_values.emplace(
        TiffTagType::StripOffsets,
        make_variant(TiffTagType::StripOffsets, this->_offset_data));
};
