...
inline std::string read_char_str(std::basic_istream<char>& stream,
                                 const size_t length) {
    if (!stream) {
...
class TiffIFD {
   ...
   public:
    // delete the default constructor, such that the user always has a valid
    // stream
    TiffIFD() = delete;

    TiffIFD(std::basic_istream<char>& stream, uint_t address,
            std::shared_ptr<VirtualEndianHandler> endian_handler)
        : _stream(stream), _address(address), _endian_handler(endian_handler) {}
    ...
    TiffTag convert_to_tag(std::array<char, TiffTag_size> data) {
        ...
        tag.DataType = this->_endian_handler->convert(
            std::array<char, 2>{data.at(2), data.at(3)});

        tag.DataCount =
            this->_endian_handler->convert(std::array<char, 4>{data.at(4),
                                                               data.at(5),
                                                               data.at(6),
                                                               data.at(7)});

        tag.DataOffset =
            this->_endian_handler->convert(std::array<char, 4>{data.at(8),
                                                               data.at(9),
                                                               data.at(10),
                                                               data.at(11)});
        return tag;
...
#if PRINT == 1
        std::cout << "TagID " << std::format("0x{:02X} ", tag.TagID)
                  << to_string(TiffTagType(tag.TagID)) << " DataType "
                  << std::format("0x{:02X} ", tag.DataType)
                  << to_string(TiffDataType(tag.DataType)) << " DataCount "
                  << std::format("0x{:04X}", tag.DataCount) << " DataOffset "
                  << std::format("0x{:04X}", tag.DataOffset) << "\n";
#endif
    }
...
#if PRINT == 1
        std::cout << "---------------------------------------------\n";
        std::cout << "TagID: " << static_cast<int>(tag.TagID) << " "
                  << to_string(tag.TagID) << "\n";

        for (auto i : tag.data) {
            std::visit([](auto el) { std::cout << to_string(el); }, i);
            std::cout << "\n";
        }
#endif
    }

    std::basic_istream<char>& get_stream() { return this->_stream; }

    TiffTag getTag(const TiffTagType tag_type) const {
        const auto ret =
            std::find_if(this->_tagList.begin(),
                         this->_tagList.end(),
        ...
        if (ret == this->_tagList.end()) {
            throw std::range_error(std::format("TiffTagType was not found {}",
                ...
                std::visit(
                    [&, this](auto& el) {
                        ...
                        if constexpr (std::is_same_v<T, std::vector<uint8_t>>) {
                            ...
                            el.push_back(
                                this->_endian_handler->convert(offset, i));
...
class TiffReadStrips {
   ...
   public:
    ...
    explicit TiffReadStrips(
        TiffIFD& ifd, std::shared_ptr<VirtualEndianHandler> endian_handler)
        : _ifd(ifd),
          _stream(ifd.get_stream()),
          _endian_handler(endian_handler) {}
    ...
    TiffTagRead get_metadata_tag(TiffTagType tag, const size_t size) const {
        ...
        if (res.data.size() != size) {
            throw std::runtime_error(
                std::format("Error tag data size is {} expected {}\n",
                            res.data.size(),
                            size));
        }
    ...
    std::string readStrips() {
        ...
        // TiffTag getTag
        // ImageX
        const TiffTagRead image_width =
            this->get_metadata_tag(TiffTagType::ImageWidth, 1);
        const TiffTagRead image_length =
            this->get_metadata_tag(TiffTagType::ImageLength, 1);
        const TiffTagRead strip_byte_count =
            this->get_metadata_tag(TiffTagType::StripByteCounts, 1);
        const TiffTagRead strip_offset =
            this->get_metadata_tag(TiffTagType::StripOffsets, 1);
        const TiffTagRead rows_per_strip =
        ...
        if (samples_per_pixel.data[0] != one) {
            const TiffTagRead planar_config =
                this->get_metadata_tag(TiffTagType::PlanarConfiguration, 1);
            std::cout << "planar_config " << to_ulong(planar_config.data[0])
                      << "\n";
        }

        TiffDataVariant strips_in_image_var =
            (image_length.data[0] + rows_per_strip.data[0] - one) /
            rows_per_strip.data[0];
        uint64_t strips_in_image = to_ulong(strips_in_image_var);

        std::cout << "Data length "
                  << to_string(image_length.data[0] + rows_per_strip.data[0] -
                               one)
                  << "\n";
        ...
        for (uint64_t i = 0; i < strips_in_image; ++i) {
            if (i < strip_offset.data.size()) {
                this->_stream.seekg(static_cast<std::streamoff>(
                    to_size_t(strip_offset.data.at(i))));
                size_t bytes_read =
                    static_cast<size_t>(to_ulong(strip_byte_count.data[0]));
...
class TiffReadStrips_RGB : public TiffReadStrips {
    TiffReadStrips_RGB(TiffIFD& ifd,
                       std::shared_ptr<VirtualEndianHandler> endian_handler)
        : TiffReadStrips(ifd, endian_handler) {}
};
