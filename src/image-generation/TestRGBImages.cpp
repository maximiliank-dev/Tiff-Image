...
TestImageSingleColor::TestImageSingleColor(size_t n, size_t m, size_t pixel, uint8_t color) : 
    _color(color), _pixel(pixel), _rows(n), _columns(m), TestAbstractImage<uint8_t>(m, TestImageSingleColor::pixel_size) {
}
...
inline std::array<uint8_t, 3> TestImageRampRGB::calc_function(const size_t i, const size_t j, const double theta,
     ...
     * the max is for a given θ at the points: [ (0,0), (0,y_max), (x_max, 0), (x_max, y_max) ] 
     * since the function is linear and constant for a given θ
     */
    ...
    for(size_t i = 0; i < array_size; i++) {
        pi[i] = std::get<0>(points[i]) * std::cos(theta) + std::get<1>(points[i]) * std::sin(theta);
    }
    ...
    for(size_t it = 0; it < 3; it++) {
        if(this->_idx[it]) {
            double px = static_cast<double>(i)* std::cos(theta) + static_cast<double>(j) * std::sin(theta);
...
void TestImageRampRGB::generateImage() {
    ...
    const uint8_t rows = (this->_rows <= std::numeric_limits<uint8_t>::max())?
        static_cast<uint8_t>(this->_rows): std::numeric_limits<uint8_t>::max();
    const uint8_t cols = (n <= std::numeric_limits<uint8_t>::max())?
        static_cast<uint8_t>(n): std::numeric_limits<uint8_t>::max();

    for(size_t i = 0; i < this->_rows; i++ ) {
        v = std::vector<uint8_t>(n, uint8_t{0});
        for(size_t j = 0; j < n; j += this->img->get_pixel_number_of_colors() ) {
            //j/3 is because each pixel has 3 values
            std::array<uint8_t, 3> dt = this->calc_function(i, j/TestImageRampRGB::pixel_size, theta, 
                rows, 
                cols);
            for(size_t k = 0; k < 3; ++k ) {
                v[j+k] = dt[k];
